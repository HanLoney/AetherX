...
import { ArrowRight, Eye, EyeOff, Link2, LockKeyhole, Server, UserRound } from "@lucide/vue";
...
const mode = ref<"login" | "pair">("login");
...
<template>
  <main class="login-page">
    ...
    <form class="login-sheet" @submit.prevent="submit">
      ...
      <div class="mode-tabs"><button type="button" :class="{active:mode==='login'}" @click="mode='login'">账号登录</button><button type="button" :class="{active:mode==='pair'}" @click="mode='pair'">配对电脑</button></div>
      <div v-if="mode==='login'" class="login-fields">
        <div class="field icon-field">
          <label for="server">电脑端地址</label>
          <div><Server :size="18" /><input id="server" v-model="serverUrl" inputmode="url" autocomplete="url" /></div>
        </div>
        <div class="field icon-field">
        ...
        <div class="field icon-field">
          ...
          <div>
            <LockKeyhole :size="18" />
            <input id="password" v-model="password" :type="showPassword ? 'text' : 'password'" autocomplete="current-password" placeholder="输入密码" />
      ...
      <p v-if="session.error.value" class="error-banner">{{ session.error.value }}</p>
      <button class="primary-button login-button" type="submit" :disabled="session.busy.value || (mode==='login' ? !username.trim() || !password : !pairingCode.trim())">
        <span>{{ session.busy.value ? (mode==='pair'?'等待电脑确认…':'正在连接…') : (mode==='pair'?'申请配对':'进入 AetherX') }}</span><ArrowRight :size="18" />
      </button>
...
.mode-tabs{display:grid;grid-template-columns:1fr 1fr;margin:24px 0 0;padding:4px;border-radius:15px;background:rgba(118,110,141,.07)}.mode-tabs button{height:38px;border:0;border-radius:11px;color:#8b8597;background:transparent;font-size:11px;font-weight:700}.mode-tabs button.active{color:#544f6c;background:rgba(255,255,255,.92);box-shadow:0 7px 18px rgba(86,79,112,.1)}
