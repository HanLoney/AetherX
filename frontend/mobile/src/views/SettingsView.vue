...
import type { AiConfig, AiConfigInput, AiModelKind, AiModelOption, AiProviderProfile, BillingBalance, BillingModuleDescriptor, BillingPaymentSession } from "../lib/api";
...
const aiModelKindFilter = ref<"all" | AiModelKind>("all");
const aiModelPickerError = ref("");
const aiError = ref("");
const aiNotice = ref("");
...
const aiModelKindLabels: Record<AiModelKind, string> = {
  text: "文字模型",
  multimodal: "多模态",
  image: "图像生成",
  video: "视频生成",
...
const aiModelKindFilters = computed(() => {
  const counts = new Map<AiModelKind, number>();
  aiModels.value.forEach((model) => counts.set(model.kind || "unknown", (counts.get(model.kind || "unknown") || 0) + 1));
  return [
    { id: "all" as const, label: "全部", count: aiModels.value.length },
    ...Object.entries(aiModelKindLabels)
      .filter(([kind]) => counts.has(kind as AiModelKind))
      .map(([kind, label]) => ({ id: kind as AiModelKind, label, count: counts.get(kind as AiModelKind) || 0 }))
  ];
});
const filteredAiModels = computed(() => {
  const query = aiModelSearch.value.trim().toLocaleLowerCase();
  const typed = aiModelKindFilter.value === "all"
    ? aiModels.value
    : aiModels.value.filter((model) => (model.kind || "unknown") === aiModelKindFilter.value);
  const matches = query
    ? typed.filter((model) =>
...
function openAiModelPicker() {
  ...
  aiModelSearch.value = "";
  aiModelKindFilter.value = "all";
  aiModelPickerError.value = "";
...
function selectAiModel(model: AiModelOption) {
  if (model.selectableForChat === false) {
    aiModelPickerError.value = `${modelKindLabel(model.kind)}不能作为 AetherX 对话模型使用。`;
    return;
  }
  aiForm.model = model.id;
  closeAiModelPicker();
  aiNotice.value = `已选择 ${model.name || model.id}（${modelKindLabel(model.kind)}）。`;
}

function modelKindLabel(kind: AiModelKind | undefined) {
  return aiModelKindLabels[kind || "unknown"];
}

function selectAiModelKind(kind: "all" | AiModelKind) {
  aiModelKindFilter.value = kind;
  aiModelPickerError.value = "";
}
...
async function fetchAiModels() {
  ...
  try {
    ...
    const firstChatModel = aiModels.value.find((model) => model.selectableForChat !== false);
    if (!aiForm.model.trim() && firstChatModel) aiForm.model = firstChatModel.id;
    const typeCount = new Set(aiModels.value.map((model) => model.kind || "unknown")).size;
    aiNotice.value = `已获取 ${aiModels.value.length} 个模型，识别出 ${typeCount} 类能力。`;
...
function collectAiConfigInput(): AiConfigInput | null {
  ...
  const knownModel = aiModels.value.find((item) => item.id === model);
  if (knownModel?.selectableForChat === false) {
    aiError.value = `${modelKindLabel(knownModel.kind)}不能保存为对话模型，请选择文字或多模态模型。`;
    return null;
  }
  if (!aiForm.apiKey.trim() && !aiKeyCanRemain.value) {
  ...
  <AppShell title="" headerless>
    ...
    <section class="settings-list">
      ...
      <article v-else>
        ...
        <b :class="{ warning: !aiState?.hasApiKey }">{{ aiState?.hasApiKey ? '就绪' : '桌面配置' }}</b>
      </article>
    </section>
    ...
    <Teleport to="body">
      ...
      <Transition name="fade">
        <div v-if="aiEditorOpen" class="sheet-backdrop" @click.self="closeAiEditor">
          <form class="ai-settings-sheet" role="dialog" aria-modal="true" aria-label="AI 接入设置" @submit.prevent="saveAiConfig">
            <div class="sheet-handle" />
            <header>
            ...
            <div class="ai-editor-fields">
              ...
              <label>
                ...
                <button v-if="aiModels.length" class="ai-model-picker-trigger" type="button" :disabled="aiSaving || aiModelsLoading" @click="openAiModelPicker">
                  ...
                  <span><small>从 {{ aiModels.length }} 个模型中选择<span v-if="selectedAiModel"> · {{ modelKindLabel(selectedAiModel.kind) }}</span></small><strong>{{ selectedAiModel?.name || aiForm.model || '请选择模型' }}</strong></span>
                  <ChevronRight :size="17" />
                </button>
          ...
          <section class="ai-model-picker-sheet" role="dialog" aria-modal="true" aria-label="选择 AI 模型" @keydown.esc="closeAiModelPicker">
            ...
            <nav class="ai-model-kind-tabs" aria-label="模型类型筛选">
              <button
                ...
                :key="filter.id"
                type="button"
                :class="{ active: aiModelKindFilter === filter.id }"
                @click="selectAiModelKind(filter.id)"
              >
            ...
            <div class="ai-model-list">
              <button
                ...
                :key="model.id"
                type="button"
                :class="{ selected: model.id === aiForm.model, unavailable: model.selectableForChat === false }"
              ...
              >
                ...
                <span class="ai-model-copy">
                  ...
                  <code>{{ model.id }}</code>
                  <span>
                    <b :class="['ai-model-kind-badge', model.kind || 'unknown']">{{ modelKindLabel(model.kind) }}</b>
                    <b v-if="model.ownedBy">{{ model.ownedBy }}</b>
                    <b v-if="model.contextLength">{{ modelContextLabel(model.contextLength) }}</b>
                  </span>
                </span>
                <i class="ai-model-check">
                  <Check v-if="model.id === aiForm.model" :size="16" />
                  <span v-else-if="model.selectableForChat === false">仅展示</span>
                </i>
              </button>
...
.ai-model-picker-backdrop{z-index:80;background:rgba(39,36,54,.34);backdrop-filter:blur(10px)}.ai-model-picker-sheet{width:100%;height:min(78dvh,720px);display:flex;flex-direction:column;overflow:hidden;padding:12px 15px calc(15px + env(safe-area-inset-bottom));border:1px solid rgba(255,255,255,.82);border-bottom:0;border-radius:30px 30px 0 0;background:radial-gradient(circle at 94% 2%,rgba(var(--blue-rgb),.18),transparent 31%),radial-gradient(circle at 2% 94%,rgba(var(--pink-rgb),.12),transparent 36%),rgba(249,249,252,.99);box-shadow:0 -28px 78px rgba(53,48,72,.27)}.ai-model-picker-sheet header{display:flex;align-items:flex-start;justify-content:space-between;flex:0 0 auto;padding:2px 3px 0}.ai-model-picker-sheet header>div{min-width:0}.ai-model-picker-sheet header span{color:#8a82aa;font-size:calc(7px * var(--font-scale,1));font-weight:850;letter-spacing:.16em}.ai-model-picker-sheet h2{margin:3px 0 0;color:#4e4958;font-size:calc(21px * var(--font-scale,1));letter-spacing:-.045em}.ai-model-picker-sheet header p{overflow:hidden;margin:5px 0 0;color:#9a93a1;font-size:calc(7px * var(--font-scale,1));text-overflow:ellipsis;white-space:nowrap}.ai-model-picker-sheet header>button{width:38px;height:38px;display:grid;place-items:center;flex:0 0 auto;padding:0;border:0;border-radius:13px;color:#817a8b;background:rgba(111,103,136,.07)}
.ai-model-search{height:47px;min-height:47px;display:flex;align-items:center;gap:9px;flex:0 0 47px;margin:14px 2px 11px;padding:0 11px;border:1px solid rgba(112,104,137,.12);border-radius:15px;background:rgba(255,255,255,.82);box-shadow:0 9px 24px rgba(75,70,103,.055)}.ai-model-search>svg{flex:0 0 auto;color:#9189a4}.ai-model-search input{min-width:0;flex:1;height:100%;padding:0;border:0;outline:0;color:#55505f;background:transparent;font-size:calc(10px * var(--font-scale,1))}.ai-model-search input::-webkit-search-cancel-button{display:none}.ai-model-search button{width:28px;height:28px;display:grid;place-items:center;padding:0;border:0;border-radius:9px;color:#9a91a2;background:rgba(112,104,137,.07)}
.ai-model-kind-tabs{min-height:38px;display:flex;align-items:center;gap:7px;overflow-x:auto;flex:0 0 auto;margin:0 1px 10px;padding:1px 1px 3px;scrollbar-width:none}.ai-model-kind-tabs::-webkit-scrollbar{display:none}.ai-model-kind-tabs button{min-height:34px;display:flex;align-items:center;gap:5px;flex:0 0 auto;padding:7px 9px;border:1px solid rgba(112,104,137,.1);border-radius:999px;color:#827a8b;background:rgba(255,255,255,.7);font-size:calc(7px * var(--font-scale,1));font-weight:750;white-space:nowrap}.ai-model-kind-tabs button b{min-width:17px;padding:2px 5px;border-radius:999px;color:#9a91a3;background:rgba(112,104,137,.07);font-size:calc(6px * var(--font-scale,1));text-align:center}.ai-model-kind-tabs button.active{border-color:rgba(126,144,186,.24);color:#6c6e98;background:linear-gradient(125deg,rgba(var(--pink-rgb),.12),rgba(var(--blue-rgb),.15))}.ai-model-kind-tabs button.active b{color:#716c94;background:rgba(255,255,255,.58)}.ai-model-picker-error{display:flex;align-items:center;gap:6px;flex:0 0 auto;margin:0 2px 9px;padding:8px 10px;border-radius:11px;color:#a65f72;background:rgba(202,103,132,.09);font-size:calc(7px * var(--font-scale,1));line-height:1.4}.ai-model-picker-error svg{flex:0 0 auto}
