...
test("启动器界面提供完整的安装、启动、停止与监控入口", () => {
  ...
  assert.match(html, /data-monitor-node="desktop"/);
  assert.match(html, /data-monitor-node="remote"/);
  assert.match(html, /data-monitor-hub-process/);
  assert.match(html, /data-monitor-mobile-state/);
