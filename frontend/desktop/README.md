...
在“连接手机 / 设备管理”中优先选择“一次配对客户端与手机 Hub”。桌面端只把 USB 回环、局域网、Anywhere 候选地址、客户端与 Hub 会话 ID、高熵一次性密钥和过期时间放入 v2 短码；手机扫描后并发探测候选入口，自动选中第一个可用地址，再从电脑 Hub 拉取临时公钥、节点标识与候选入口。两项申请随后并行提交，用户不需要填写地址，在电脑端批准一次即可完成授权。底层凭据仍保持隔离，客户端令牌不能代替 Peer 凭据。
