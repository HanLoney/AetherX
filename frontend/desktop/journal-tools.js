      ...
      inputSchema: objectSchema(
        {
          ...
          content: stringField(
            "以第一人称写下的完整正文。想配图时在合适位置单独一行写占位符（必须独占一行才生效），最多两张，自己决定画什么：画面里有你自己就写 [[自拍: 画面描述]]（会参考你的人设图，保持形象一致），只画场景/物件/风景写 [[配图: 画面描述]]。描述必须是能直接画出来的具体画面（场景、你的动作神态、光线、构图），绝不能写「我想象的画面」这类空泛的话；画面要贴合这篇手记真实写到的内容。画自拍时不要描述自己的外貌（发型、发色、脸型、身材、服装等），你的样子由人设图决定，写出来反而会和人设图冲突，只写你在做什么、什么神态、场景与光线。你只能稳定画出你自己，画不出用户或其他真人，别画含他人的「两个人」画面；优先用自拍把自己画进去。不要在正文句子里复述或举例这套占位符语法本身，否则会被误当成真配图生成。所有图都会以二次元动漫风格生成。不要自己写 Markdown 图片链接或图片网址。"
          )
