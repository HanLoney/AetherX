function registerAlbumRoutes(router, service) {
  router.add("GET", "/api/v1/album/moments", ({ userId, query }) => ({
    data: service.listMoments(userId, query)
  }), { module: "anniversary-album" });
  router.add("POST", "/api/v1/album/moments", ({ userId, body, requestId }) => {
    const result = service.createMomentWithRequestId(userId, body, requestId);
    return { status: result.status, data: result.result };
  }, { module: "anniversary-album" });
  router.add("GET", "/api/v1/album/moments/:id", ({ userId, params }) => ({
    data: service.getMoment(userId, params.id)
  }), { module: "anniversary-album" });
  router.add("PATCH", "/api/v1/album/moments/:id", ({ userId, params, body, requestId }) => {
    const result = service.updateMomentWithRequestId(
      userId,
      params.id,
      body,
      requestId
    );
    return { status: result.status, data: result.result };
  }, { module: "anniversary-album" });
  router.add("POST", "/api/v1/album/moments/:id/hide", ({ userId, params, requestId }) => {
    const result = service.hideMomentWithRequestId(userId, params.id, requestId);
    return { status: result.status, data: result.result };
  }, { module: "anniversary-album" });
  router.add(
    "POST",
    "/api/v1/album/moments/:id/sources",
    ({ userId, params, body, requestId }) => {
      const result = service.addSourceWithRequestId(
        userId,
        params.id,
        body,
        requestId
      );
      return { status: result.status, data: result.result };
    },
    { module: "anniversary-album" }
  );
  router.add("DELETE", "/api/v1/album/moments/:id", ({ userId, params, requestId }) => {
    const result = service.deleteMomentWithRequestId(userId, params.id, requestId);
    if (result.status !== 204) return { status: result.status, data: result.result };
    return { status: 204 };
  }, { module: "anniversary-album" });
  router.add("GET", "/api/v1/album/source-candidates", ({ userId, query }) => ({
    data: service.listSourceCandidates(userId, query)
  }), { module: "anniversary-album" });
}

module.exports = { registerAlbumRoutes };
