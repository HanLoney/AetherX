...
| --- | --- | --- |
| `read` | 查询待办 | 自动执行 |
| `write` | 新建、修改、完成待办 | 按授权偏好自动执行或聊天内确认 |
| `destructive` | 删除待办 | 按授权偏好执行，并明确展示影响 |

工具状态始终显示在聊天流中。关闭“自动同意工具调用”后，写入和删除操作
使用聊天卡片内的允许/拒绝按钮，不使用系统弹窗。用户拒绝后，Agent Loop
仍向模型回传结构化失败结果，模型不得声称操作已成功。
