...
首次打开时创建第一个账号。第一个账号会在同一数据库事务中认领旧版 `local-user` 数据，不会清空已有内容。后续账号只有在 Hub 配置了注册口令时才能创建。
...
Android 客户端支持账号登录和一次性电脑配对。真机调试、Keystore 凭证保护与网络限制见[移动端说明](frontend/mobile/README.md)。

### 自托管 Hub

生产环境应使用独立数据目录、固定主密钥、注册口令和 HTTPS 反向代理。不要把 `4318` 端口直接暴露到公网。
